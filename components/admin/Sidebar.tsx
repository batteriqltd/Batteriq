...
import {
  ...
  MessageSquare, Mail, LogOut, ChevronRight, CreditCard,
...
function LiveClock() {
  ...
  useEffect(() => {
    function update() {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-KE', {
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit',
        hour12: true,
...
export function AdminSidebar({ adminName = 'Batteriq Admin', adminRole = 'super_admin' }: {
  ...
  return (
    <aside
      className="w-[260px] min-h-screen flex flex-col flex-shrink-0 fixed left-0 top-0 bottom-0 z-30 overflow-y-auto"
    ...
    >
      ...
      <nav className="flex-1 px-4 py-6 space-y-1">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] px-3 mb-4" style={{ color: 'rgba(255,255,255,0.3)' }}>
          Management
        </p>
        {NAV.map(item => {
          ...
          return (
            <Link
              ...
              style={{
                ...
                boxShadow: active ? '0 8px 24px rgba(0,0,255,0.25)' : 'none',
              }}
            ...
            >
              ...
              <span className="text-[14px] font-semibold flex-1" style={{ color: active ? '#ffffff' : '#cbd5e1' }}>
                {item.label}
              </span>
              {item.badge === 'live' && (
      ...
      <div className="px-4 pb-4 border-t pt-6" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        <p className="text-[10px] font-black uppercase tracking-[0.2em] px-3 mb-3" style={{ color: 'rgba(255,255,255,0.3)' }}>
          Export Data
        </p>
      ...
      <div className="px-4 pb-6 border-t pt-6" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        <div
          className="flex items-center gap-3 px-4 h-[64px] mb-3 rounded-2xl border border-white/5"
          style={{ background: 'rgba(255,255,255,0.03)' }}
        >
          <div
            className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 font-black text-sm text-[#00004d]"
            style={{ background: '#ffffff' }}
          >
